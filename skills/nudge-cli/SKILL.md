...
- This skill targets CLI 0.2.1 or newer (`document template projects` requires 0.3.0 or newer). Help and `nudge --version` work offline; upgrade an older CLI before using these commands.
...
- Multi-line text takes a `--description-file`/`--content-file`/`--body-file` PATH, or `-` for stdin, instead of an inline flag. Encode in-app entity chips in that Markdown as marked relative links (`#nudge-issue` / `#nudge-project` / `#nudge-document`); live metrics use `#nudge-widget` — see [argument handling](references/arguments.md#internal-entity-mentions-in-markdown) and [live dashboard widgets](references/arguments.md#live-dashboard-widgets-in-markdown).
...
| Build/edit mixed-content dashboards, write or organize documents, database schemas/records, revisions, archive/restore, list vs. search | [Workflows: documents](references/workflows.md#documents) |
